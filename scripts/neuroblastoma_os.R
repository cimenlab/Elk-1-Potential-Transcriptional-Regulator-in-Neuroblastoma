...
##              ROC for overall survival (OS), correlation of ELK1 with the
##              focus markers and a wider marker panel, violin plots, and MYCN
##              expression across cell lines. Part C: the same pipeline run
##              independently for each PEA3 gene (ETV1, ETV4, ETV5), without
##              averaging."
##
...
if (ELK1_SPLIT=="optimal" && requireNamespace("survminer",quietly=TRUE) &&
    sum(!is.na(clin$os_time) & !is.na(clin$os_event)) > 20) {
  cut_df <- data.frame(t=clin[samples,"os_time"], e=clin[samples,"os_event"], ELK1=elk1v)
...
## ---- survival ------------------------------------------------------------------
have_os <- sum(!is.na(clin$os_time) & !is.na(clin$os_event)) > 10

if (have_os) {
  # Kaplan-Meier (OS)
  kmo <- clin %>% dplyr::filter(!is.na(os_time), !is.na(os_event), !is.na(ELK1_group))
  fit_os <- survminer::surv_fit(survival::Surv(os_time, os_event) ~ ELK1_group, data = kmo)
  
  p_os <- survminer::ggsurvplot(
    fit_os, data = kmo, pval = TRUE, conf.int = TRUE, risk.table = TRUE,
    palette = unname(PAL_ELK1), title = "Overall Survival by ELK1 - GSE62564",
    xlab = "Time (months)", ylab = "OS probability", ggtheme = theme_pub(11)
  )
  add_fig("03_KM_OS_ELK1", function() print(p_os), 8, 8)
  
  # Time-dependent ROC (OS)
  roc_df <- kmo %>% dplyr::filter(!is.na(ELK1_expr))
  if (nrow(roc_df) > 30) {
    tmax <- suppressWarnings(max(roc_df$os_time[roc_df$os_event == 1], na.rm = TRUE))
      ...
      if (!is.null(roc)) {
        ...
        add_fig("07_TimeROC", function() {
          plot(
            ...
            main = "ELK1 time-dependent ROC (OS) - GSE62564"
          )
          if (length(tt_pts) > 1) {
  ...
  cat("OS data insufficient/missing - survival analysis skipped.\n")
}
## ---- markers_setup ------------------------------------------------------------------
read_markers <- function(file) {
  if (!file.exists(file)) return(character(0))
...
## Optional (disabled): uncomment to generate multi-page violin PDFs for the marker lists.
...
# (no averaged score): classification, OS, ROC, correlation and violin
...
## ############################################################################
## PEA3 - PATIENTS (GSE62564, n~498): independent classification + OS + ROC +
...
## (classification / OS / ROC / correlation / violin)
run_pea3_gene_analysis <- function(gene) {
  ...
  if (ELK1_SPLIT == "optimal" && sum(!is.na(clin$os_time) & !is.na(clin$os_event)) > 20) {
    cut_df <- data.frame(t = clin[samples, "os_time"], e = clin[samples, "os_event"], G = gene_v)
    cut_df <- cut_df[!is.na(cut_df$t) & !is.na(cut_df$e), ]
  ...
  if (sum(!is.na(clin_g$os_time) & !is.na(clin_g$os_event)) > 10) {
    kmo <- clin_g %>% dplyr::filter(!is.na(os_time), !is.na(os_event), !is.na(.data[[paste0(gene,"_group")]]))
    kmo$Group <- factor(kmo[[paste0(gene, "_group")]], levels = grp_levels)
    fit_os <- survminer::surv_fit(survival::Surv(os_time, os_event) ~ Group, data = kmo)
    p_os <- survminer::ggsurvplot(fit_os, data = kmo, pval = TRUE, conf.int = TRUE, risk.table = TRUE,
                                  palette = unname(pal_g[grp_levels]),
                                  title = sprintf("Overall Survival by %s - GSE62564", gene),
                                  xlab = "Time (months)", ylab = "OS probability", ggtheme = theme_pub(11))
    print(p_os)
    add_fig(sprintf("07_KM_OS_%s", gene), function() print(p_os), 8, 8)
  } else {
    message("OS data insufficient - KM skipped (", gene, ").")
  }
  
  ## -- Time-dependent ROC (OS) --
  roc_df <- clin_g %>% dplyr::filter(!is.na(.data[[paste0(gene,"_expr")]]), !is.na(os_time), !is.na(os_event))
  if (nrow(roc_df) > 30) {
    tmax <- suppressWarnings(max(roc_df$os_time[roc_df$os_event == 1], na.rm = TRUE))
    tt_pts <- c(12, 36, 60); tt_pts <- tt_pts[is.finite(tmax) & tt_pts < tmax]
    if (length(tt_pts) >= 1) {
      roc <- tryCatch(
        timeROC::timeROC(T = roc_df$os_time, delta = roc_df$os_event, marker = roc_df[[paste0(gene,"_expr")]],
                         cause = 1, times = tt_pts, iid = FALSE),
        error = function(e) { message("timeROC error: ", conditionMessage(e)); NULL })
      if (!is.null(roc)) {
        tcols <- paste0("t=", tt_pts)
        cols  <- c("#4393c3", "#d6604d", "#1a9850")[seq_along(tt_pts)]
        add_fig(sprintf("08_TimeROC_OS_%s", gene), function() {
          plot(roc$FP[, tcols[1]], roc$TP[, tcols[1]], type = "l", col = cols[1], lwd = 2,
               xlim = c(0, 1), ylim = c(0, 1), xlab = "1 - Specificity", ylab = "Sensitivity",
               main = sprintf("%s time-dependent ROC (OS) - GSE62564", gene))
          if (length(tt_pts) > 1) for (i in 2:length(tt_pts)) lines(roc$FP[, tcols[i]], roc$TP[, tcols[i]], col = cols[i], lwd = 2)
          abline(0, 1, lty = 3, col = "grey60")
          legend("bottomright", bty = "n", col = cols, lwd = 2, legend = sprintf("%d mo: AUC=%.2f", tt_pts, roc$AUC[tcols]))
        }, 6, 6)
        cat(sprintf("Time-ROC (OS) AUC - %s:\n", gene)); print(round(roc$AUC, 3))
      }
    } else message("Time points exceed follow-up duration - ROC skipped (", gene, ").")
  } else message("OS + ", gene, " data insufficient - ROC skipped.")
